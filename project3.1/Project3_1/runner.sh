...
# single number to standard output.
answer_2() {
    grep -c -E "^(.*,){6}.*Bob Marley.*" ./million_songs_metadata.csv # | cut -d , -f 1 | sort | uniq -c | wc -l
    #grep -E "(\w,){6}.*Adelitas Way*," test1.txt | wc -l
    echo
}
...
# single number to standard output.
answer_3() {
    awk 'BEGIN{FS = ","} $7 ~ /The Beatles/ {total+=1} END{print total}' million_songs_metadata.csv
}
...
# single number to standard output.
answer_4() {
    awk -f question4.txt million_songs_metadata.csv
}
...
answer_5() {
    awk -f question5.txt million_songs_sales_data.csv million_songs_metadata.csv > million_songs_metadata_and_sales.csv
    #awk -f question5.txt million_songs_sales_data.csv test1.txt > million_songs_metadata_and_sales.csv
}
...
answer_6() {
    awk -f question6.txt million_songs_metadata_and_sales.csv | sort -t, -k 1nr | head -n 1 | cut -d , -f 2
}
...
# Complete the corresponding method(s) in MySQLTasks.java.
INDEX_NAME="index_songs_duration"
answer_8() {
