...
public class MySQLTasks {
    ...
    private static void q7() {
        ...
        try {
            stmt = conn.createStatement();
            String tableName = "songs";
            String sql = "SELECT track_id FROM " + tableName + " ORDER BY duration DESC LIMIT 1";
            ResultSet rs = stmt.executeQuery(sql);
            if (rs.next()) {
    ...
    private static void q8(String indexName) {
        ...
        try {
            stmt = conn.createStatement();
            String tableName = "songs";
            String sql = "alter table "+ tableName+" add index "+indexName+"(duration)";
            stmt.execute(sql);
            /*if (rs.next()) {
    ...
    private static void q10() {
        ...
        try {
            ...
            String sql = "SELECT count(*) as cnt FROM " + tableName +
                ...
                " OR binary title like '%The Beatles%' OR binary songs.release like '%The Beatles%'";
            //String sql = "SELECT count(*) as cnt FROM " + tableName + " WHERE MATCH('track_id','title','song_id','release','artist_id','artist_mbid','artist_name') AGAINST ('The Beatles')";
    ...
    private static void q11() {
        ...
        try {
            stmt = conn.createStatement();
            String tableName = "songs";
            String sql = "SELECT artist_name,count(*) as cnt FROM " + tableName + " GROUP BY artist_id ORDER BY cnt DESC LIMIT 2,1";
            ResultSet rs = stmt.executeQuery(sql);
            if (rs.next()) {
