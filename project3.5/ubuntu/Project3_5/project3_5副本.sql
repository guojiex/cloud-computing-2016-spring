...
-- start query2_opt
select sum(lo_revenue), d_year, p_brand1 from 
lineorder_opt
INNER JOIN part_opt on lo_partkey=p_partkey
INNER JOIN dwdate_opt on lo_orderdate=d_datekey
...
lineorder_opt 
INNER JOIN dwdate_opt on lo_orderdate = d_datekey
INNER JOIN customer_opt on lo_custkey = c_custkey 
INNER JOIN supplier_opt on lo_suppkey = s_suppkey 
...
credentials   gzip compupdate off;
...
diststyle key distkey (p_brand1)
sortkey (p_partkey);
CREATE TABLE supplier (
  s_suppkey     	integer        not null,
  s_name        	varchar(25)    not null,
...
diststyle key distkey (lo_orderdate)
compound sortkey (lo_partkey,lo_suppkey);
-- end redshift_create_table_optimized
copy customer from 's3://awssampledb/ssbgz/customer' 
credentials 'aws_access_key_id=AKIAJQMI4I5ECU66LHDQ;aws_secret_access_key=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' gzip;
