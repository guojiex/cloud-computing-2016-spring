...
public class AzureVMApiDemo {
    ...
    public AzureVMApiDemo(String subscriptionId, String tenantID, String applicationID, String applicationKey,
            String resourceGroupName) throws Exception {
        AzureVMApiDemo.subscriptionId = subscriptionId;
        AzureVMApiDemo.tenantID = tenantID;
        AzureVMApiDemo.applicationID = applicationID;
        AzureVMApiDemo.resourceGroupName = resourceGroupName;
        AzureVMApiDemo.applicationKey = applicationKey;
        Configuration config = createConfiguration();
        resourceManagementClient = ResourceManagementService.create(config);
        storageManagementClient = StorageManagementService.create(config);
        computeManagementClient = ComputeManagementService.create(config);
    ...
    public static Configuration createConfiguration() throws Exception {
        ...
        String token = AuthHelper.getAccessTokenFromServicePrincipalCredentials(basicURI, endpointURL, tenantID,
                applicationID, applicationKey).getAccessToken();
     ...
     *            a new name for your virtual machine [customized], will create
     *            a new one if not already exist
     * @param vmName:
     *            a PUBLIC UNIQUE name for virtual machine
     * @param resourceGroupNameWithVhd:
     *            the resource group where the storage account for VHD is copied
     * @param sourceVhdUri:
     ...
     */
    public static ResourceContext createVM(String resourceGroupName, String vmName, String resourceGroupNameWithVhd,
            String sourceVhdUri, String instanceSize, String subscriptionId, String storageAccountName,
            HashMap<String, String> tagsValue) throws Exception {

        ResourceContext contextVhd = new ResourceContext(region, resourceGroupNameWithVhd, subscriptionId, false);
        ResourceContext context = new ResourceContext(region, resourceGroupName, subscriptionId, false);
        ...
        if (context.getNetworkInterface() == null) {
            ...
            try {
                networkResourceProviderClient.getVirtualNetworksOperations()
            ...
            System.out.println(resourceGroupName + " " + context.getPublicIpName() + " " + vmName);
            NetworkHelper.updatePublicIpAddressDomainName(networkResourceProviderClient, resourceGroupName,
    ...
    public static String checkVM(ResourceContext context, String vmName) {
        ...
        try {
            VirtualMachine vmHelper = ComputeHelper.createVM(resourceManagementClient, computeManagementClient,
                    networkResourceProviderClient, storageManagementClient, context, vmName, "ubuntu", "Cloud@123")
                    .getVirtualMachine();

            System.out.println("[15319/15619] " + vmHelper.getName() + " Is Created :)");
            while (ipAddress == null) {
                PublicIpAddressGetResponse result = networkResourceProviderClient.getPublicIpAddressesOperations()
                        .get(context.getResourceGroupName()/*resourceGroupName*/, context.getPublicIpName());
                ipAddress = result.getPublicIpAddress().getIpAddress();
                //System.out.println("DNS in checkVM"+result.getPublicIpAddress().getDnsSettings().getDomainNameLabel());
                Thread.sleep(10);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    // String seed = String.format("%d%d", (int)
    // System.currentTimeMillis()%1000, (int)(Math.random()*1000));
    ...
    // sourceVhdUri =
    // String.format("https://%s.blob.core.windows.net/system/Microsoft.Compute/Images/vhds/%s",
    // storageAccountName, args[2].trim());
