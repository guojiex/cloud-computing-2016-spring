...
public class DataCenterInstance {
    ...
    public boolean isHealth() {
        String path = String.format("%s/lookup/random", this.url);
        URL url;
        try {
            url = new URL(path);
            ...
            urlcon.connect();
            return urlcon.getResponseCode() == 200/* && (this.getCpuUsage() >= 0)*/;
        } catch (IOException e) {
            e.printStackTrace();
            return false;
        }
    ...
    public double getCpuUsage() throws IOException {
        ...
        try {
            ...
            in.close();
            String temp = response.toString().substring(response.toString().indexOf("<body>")).split(">")[1]
                    .split("<")[0];
            if (temp.isEmpty()) {
